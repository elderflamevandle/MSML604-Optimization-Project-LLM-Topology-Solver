...
| **Vidur Traces** | `data/vidur_traces/` | Production LLM serving traces | Enterprise-scale arrival rate modeling |

Download scripts:
```bash
python data/download_sharegpt.py      # ~100 MB
python data/download_vidur_traces.py  # ~50 MB
```
...
**Note:** the ShareGPT and Vidur traces can be fed into the optimizer as distribution samples (sample-average approximation) to produce a policy robust to workload variation — this is the proposed **stochastic LP extension** in the roadmap.
...
The following plots were generated by running `python run_all.py` (FlexGen + Helix + Vidur full pipeline):

### Overall Throughput Comparison (FlexGen vs Helix vs Vidur)
![Overall Throughput Comparison](../analysis/plots/overall_throughput_comparison.png)

*Compares the three optimization tracks: FlexGen's LP-predicted single-node throughput, Helix's MILP-optimized multi-GPU pipeline flow, and Vidur's Bayesian-optimized QPS under P99 latency constraints.*

### Optimizer Search Comparison (Bayesian vs Grid — Vidur track)
![Optimizer Comparison](../analysis/plots/optimizer_comparison.png)

*Bayesian Optimization (Optuna/TPE) converges to the same optimum as exhaustive grid search but with ~10× fewer evaluations. This motivates the proposed O5 BoOptunaOptimizer addition for the FlexGen track.*
